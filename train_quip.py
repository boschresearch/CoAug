...
from tallor.quip_framework import IEFramework
...
def main():
    ...
    ## new arg for "extra" exepriment in ncbi
    parser.add_argument("--quip_train", type=int, default=400, 
            help='the steps to train quip in each iteration of CoAug process')
    ## File parameters
    ...
    parser.add_argument('--seed', type=int, default=0,
            help='set seed for the experiment')
    ...
    parser.add_argument('--num_steps', default=30, type=int,
            help='number of steps for co-training')
    ...
    ## QuIP few-shot model
    parser.add_argument('--fewshot_N', default=20, type=int,
            help='number of seed examples for few shot setting')
    parser.add_argument('--fewshot_batch_size', default=8, type=int,
            help='batch size for few shot setting')
    ...
    parser.add_argument('--quip_init', action='store_false',
           help='initialize with QuIP pre-trained weights or not.')
    parser.add_argument('--stop_constraint', type=int, default=1000,
           help='iteration to stop training constraint model.')
    ...
    parser.add_argument('--mode', default='coaug', type=str,
           help='quip or coaug', choices=['quip', 'coaug'])
    ...
    if encoder_name == 'bert':
        ...
        opt.embedding_size = sentence_encoder.bert.config.hidden_size
    ...
    exp_dir = os.path.join(BASE_EXP_DIR, opt.dataset, opt.mode, 'QuIP', str(opt.seed), ts)
    ...
    val_data_loader, _ = get_loader(root, opt.val, sentence_encoder, batch_size, label_id_mapper, is_train=False, opt=opt)
    ...
    framework = IEFramework(label_id_mapper, dataset_name, opt, logger, batch_size, fewshot_batch_size,
                                ...
                                mode=opt.mode, fewshot_N=opt.fewshot_N, exp_dir=exp_dir)
    ...
    if model_name == 'JointIE':
        ...
        fewshot_model = QuIP([label if label!='' else 'O' for label in label_id_mapper.all_labels()], init=opt.quip_init, ner_label=label_id_mapper)
    ...
    if not opt.only_test:
        framework.train(model, prefix, fewshot_model, fewshot_prefix, save_ckpt=ckpt, fewshot_save_ckpt=fewshot_ckpt, val_step=opt.val_step, 
                        val_iter=opt.val_iter, warmup_step=opt.warmup_step, update_threshold=opt.update_threshold,
                        num_steps=opt.num_steps, load_ckpt=opt.load_ckpt, load_fewshot_ckpt=opt.load_fewshot_ckpt)
    else:
        ckpt = opt.load_ckpt
        fewshot_ckpt = opt.load_fewshot_ckpt

    if opt.mode != 'quip':
        framework.eval(model, opt.test_iter, -1, ckpt=ckpt)
        framework.fewshot_labeler.eval(fewshot_model, opt.test_iter, ckpt=fewshot_ckpt)
    else:
        ner_f1, precision, recall = framework.fewshot_labeler.eval(fewshot_model, opt.test_iter, ckpt=fewshot_ckpt)
