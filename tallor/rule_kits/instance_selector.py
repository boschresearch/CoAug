...
class InstanceSelector:
    ...
    def __init__(self, opt, timestamp, ner_label, exp_dir='.'):
    ...
    def score_pipeline(self, high_precision_instances, positive_list, step):

        if step==0:
            
            self.init_selector(high_precision_instances, positive_list)

            return positive_list, self.candidate_set

        self.logger.info('Begin selecting instances...')
    ...
    def init_selector(self, high_precision_instances, positive_list):
    ...
    def update_threshold(self, high_precision_instances):
        ...
        for label, instance_set in high_precision_instances.items():
            ...
            for instance in sampled_instances:
                ...
                if instance not in self.high_precision_repr_dict:
                    self.high_precision_repr_dict[instance] = self.encoder.encode_instance(instance)
    ...
    def global_score(self, high_pre_set, instance_repr):
        ...
        for i in range(self.global_sample_times):
            ...
            for instance in sampled_instances:
                if instance not in self.high_precision_repr_dict:
                    self.high_precision_repr_dict[instance] = self.encoder.encode_instance(instance)
                repr_list.append(self.high_precision_repr_dict[instance])
            
            sampled_repr = np.mean(np.array(repr_list), axis=0)
...
def set_logger(exp_dir, timestamp):
    ...
    file_handler = logging.FileHandler(f'{exp_dir}/logging/InstanceSelector-{timestamp}.log')
