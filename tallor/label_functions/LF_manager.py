    ...
    def __init__(self, ner_label, dataset, rule_types='composed', mode='training'):
        ...
        if mode != 'training':
            ...
            if dataset == 'conll2003':
                ...
                if rule_types == 'composed':
                    self.rule_templates.append(NCBI_SurfaceForm(ner_label))
                    self.rule_templates.append(NCBI_ComposedRule(ner_label))
                
                self.pre_match_templates += self.rule_templates
                self.pre_match_templates += [NCBI_CapitalRule(), NCBI_PosTagRule()]
            
            elif dataset == 'wikigold':

                if rule_types == 'composed':
                    self.rule_templates.append(Wikigold_SurfaceForm(ner_label))
                    self.rule_templates.append(Wikigold_ComposedRule(ner_label))
                
                self.pre_match_templates += self.rule_templates
                self.pre_match_templates += [Wikigold_CapitalRule(), Wikigold_PosTagRule()]
